...
available_setting = {
    ...
    "image_api_prompt": "请上传需要解析的题目图片， 由于识别耗时，请多次查询结果要解析的题目图片📷",  # 提示用户上传图片的文本
    "image_api_state_timeout": 300,  # 等待图片状态的超时时间（秒），默认5分钟
    "enable_markdown_image": True,  # 是否启用Markdown转图片功能，将API返回的分析结果转换为图片
    "image_api_active_push": True,  # 是否使用主动推送立即发送文本和图片，而不是等待用户查询
    # wechatcom的通用配置
